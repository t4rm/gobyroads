...
void updateEffects(GameState *gs)
{
    ...
    while (cursor != NULL)
    {
        ...
        {
            // The effect is no longer revelant (off-screen)
            shouldRemove = true;
        }
        else if (e && e->cooldown == 0)
        {
            // Execute the effect
        ...
        if (shouldRemove)
        {
            ...
            if (e)
                free(e); // We don't free the car from the effect 'cause we use it later. We should free the car through the carQueue when closing the game/erasing his line.
            free(cursor);
            gs->effects->size--;
        }
        else
